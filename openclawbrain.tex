...
\section{Abstract}
OpenClawBrain is a retrieval-routing system for long-lived agents built around a strict hot/cold path split. The hot path is local and bounded for latency-critical routing; the cold path runs asynchronous learning and structure updates.
...
\subsection*{4.5 10-Seed Ground-Zero Proof (March 2026)}
The ground-zero harness scales evaluation to 800 queries per seed across 10
independent seeds, with relation drift injected mid-run. Table~\ref{tab:ground-zero}
shows the core comparison.
...
\begin{tabular}{lcccc}
\toprule
Baseline & Accuracy & Context Used & Traversal Cost & Win Rate vs \texttt{full\_brain} \\
\midrule
\texttt{full\_brain}         & \textbf{0.9723} & 800   & 800   & --- \\
\texttt{vector\_rag\_rerank} & 0.8901          & 4\,000 & 6\,400 & 1/10 \\
\texttt{vector\_rag}         & 0.7966          & 800   & 800   & 0/10 \\
\texttt{heuristic\_stateful} & 0.7943          & 800   & 800   & 0/10 \\
\bottomrule
\end{tabular}
\caption{Ground-zero 10-seed proof (\texttt{proof\_10seed\_20260306}). \texttt{full\_brain} wins 10/10 seeds vs \texttt{vector\_rag} and \texttt{heuristic\_stateful}, 9/10 vs \texttt{vector\_rag\_rerank}. The nearest competitor uses $5\times$ the context and $8\times$ the traversal cost for lower accuracy.}
\label{tab:ground-zero}
\end{table}

\paragraph{Worked example: E048\,{::}\,E001.}
The ground-truth relation changes from \texttt{manages} to \texttt{depends\_on}
at step~22. \texttt{full\_brain} tracks the change at steps~22 and~38;
\texttt{heuristic\_stateful} stays stale; the vector baselines stay wrong or lag.
This is bounded benchmark evidence---mechanism proof, not a claim of
recorded-session, shadow, or online production superiority.

\subsection*{4.6 Reproduction Commands (Expected Output Paths)}
...
python3 /path/to/openclawbrain-site/scripts/plot_eval.py \
  ...
  --out figures/eval
\end{verbatim}
Expected outputs include \texttt{/tmp/ocb\_eval.json}, \texttt{/tmp/ocb\_expert\_regions/simulation\_curve.csv}, and regenerated SVGs in \texttt{figures/eval/}.
