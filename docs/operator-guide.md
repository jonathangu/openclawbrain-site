...
The local operator proof today is narrow, but it is real:
...
- Per-query learned `route_fn` mutation on the active pack during serving.
- Full live active-pack plasticity while traffic is flowing.
- Finished shadow-mode or online answer-quality proof.
...
If something smells wrong, treat rollback as a normal operator tool, not an admission of failure. The system is designed so you can inspect the active state, preview the pointer move, and back out cleanly without taking OpenClaw down with it.
