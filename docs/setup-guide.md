...
- Labels, scanner, and harvest are collected inputs to PG-only route training. They run on the learning side, not in the hot-path serve loop.
- `backgroundLearning` today means: export turns → build/refresh a candidate pack off the hot path → stage and promote → compiler serves from the newly promoted pack. This is **not** live mutation of the active pack during serving.
- Graph-dynamics fields (decay, Hebbian, prune/merge counts) are recorded **inside the immutable pack artifact** as build-time metadata. Live continuous graph update on the active pack is the **target end shape**, not the current reality.
